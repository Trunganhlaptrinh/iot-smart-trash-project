//#define BLYNK_TEMPLATE_ID "TMPL6RyDr_dAA"
// #define BLYNK_TEMPLATE_NAME "Back up for IOT"
// #define BLYNK_AUTH_TOKEN "F1Jrcw89wMcQq1b5UW5rtarqloR8KB3A"

//  Khai báo Template ID và Tên project trên nền tảng Blynk
#define BLYNK_TEMPLATE_ID "TMPL6RyDr_dAA" // AA
...
char auth[] = "F1Jrcw89wMcQq1b5UW5rtarqloR8KB3A"; // code Blynk cung cấp    3A để check nhanh
...
void loop() {
  ...
  } else if (p1 < 40) warned1 = false; // Nếu đã đổ rác → cho phép cảnh báo lại

  if (p2 >= 40 ) {
    Blynk.logEvent("full_bin2", "Thung2 da day!");
    warned2 = true;
  } else if (p2 < 40) warned2 = false;

  if (p3 >= 40 ) {
    Blynk.logEvent("full_bin3", "Thung3 da day!");
    warned3 = true;
  } else if (p3 < 40) warned3 = false;

  delay(500);  //  Đợi 1 giây trước khi đọc lần tiếp theo
}
